"""
Cricket X Tracker - Backend API

FastAPI application for serving crash game statistics.
This module provides REST API endpoints for accessing Cricket X game data
...
DATABASE_PATH: str = os.getenv("DATABASE_PATH", "cricketx.db")

# CORS allowed origins - configurable via environment
ALLOWED_ORIGINS: List[str] = [
    "http://localhost:3000",
    "http://localhost:3001",
    os.getenv("FRONTEND_URL", "https://cricketxtracker.com"),
    os.getenv("SECONDARY_DOMAIN", "https://www.cricketxtracker.com"),
]
...
app = FastAPI(
    title="Cricket X Tracker API",
    description="Real-time statistics API for SmartSoft Cricket X crash game",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
async def startup() -> None:
    ...
    """
    logger.info("Starting Cricket X Tracker API...")
    ...
    try:
        async with db_manager.connect() as db:
            ...
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_multiplier ON cricketx_rounds(crash_multiplier)"
            )
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_created ON cricketx_rounds(created_at DESC)"
            )
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_round_id ON cricketx_rounds(round_id)"
            )
...
async def get_rounds(
    ...
    try:
        # Get total count
        cursor = await db.execute("SELECT COUNT(*) FROM cricketx_rounds")
...
async def get_summary(db: aiosqlite.Connection = Depends(get_db)) -> SummaryStats:
    ...
    try:
        ...
        # Calculate median
        cursor = await db.execute("""
            SELECT crash_multiplier FROM cricketx_rounds
            ORDER BY crash_multiplier
            LIMIT 1 OFFSET (SELECT COUNT(*) FROM cricketx_rounds) / 2
        """)
...
async def get_distribution(
    ...
    try:
        ...
        for name, condition in DISTRIBUTION_BUCKETS:
            cursor = await db.execute(
                f"SELECT COUNT(*) FROM cricketx_rounds WHERE crash_multiplier {condition}"
            )
...
async def health() -> HealthResponse:
    ...
    try:
        async with db_manager.connect() as db:
            ...
            cursor = await db.execute(
                "SELECT MAX(created_at) FROM cricketx_rounds"
            )
...
crash_stats_router = create_crash_game_router(db_manager, "cricketx")
...
if __name__ == "__main__":
    ...
    logger.info("Starting Cricket X Tracker API server...")
